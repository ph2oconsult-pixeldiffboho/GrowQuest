...
export function generateUserManual() {
  ...
  doc.text("Version 5.6", 105, 250, {align:"center"});
  ...
  const toc = [
    ...
    "12. Tips for Young Adventurers",
    "13. Frequently Asked Questions",
    "14. Curriculum Alignment Reference",
  ];
  doc.setFont("helvetica","normal");
  doc.setFontSize(11);
  ...
  const features = [
    ...
    "My Story — a personalised adventure storybook that writes itself as children complete quests",
  ];
  ...
  // ── 10. MY STORY ──────────────────────────────────────────────
  y = 25;
  y = addHeader(doc, "10. My Story: Your Personal Adventure", y);
  y = addBody(doc, "Every time a child completes a quest, their animal companion writes a paragraph of a personalised adventure story. The story is unique to each child, weaving in what they actually did and which competencies they grew.", y);
  y += 2;
  y = addSubHeader(doc, "How It Works", y);
  y = addBullet(doc, "After each quest, a new paragraph appears in the storybook", y);
  y = addBullet(doc, "Every 5 paragraphs form a chapter with a title based on the realm explored", y);
  y = addBullet(doc, "The story uses the child's name and their companion's name", y);
  y = addBullet(doc, "Language is adapted to the child's age group", y);
  y = addBullet(doc, "Each paragraph ends with a hint of what might come next", y);
  y += 2;
  y = addSubHeader(doc, "Chapter Titles", y);
  y = addBullet(doc, "Echo Isles chapters: Voices on the Wind", y);
  y = addBullet(doc, "Wonder Peaks chapters: The Crystal Path", y);
  y = addBullet(doc, "Heartwood Grove chapters: Where the Trees Whisper", y);
  y += 2;
  y = addSubHeader(doc, "Accessing the Storybook", y);
  y = addBody(doc, 'Tap the "My Story" button on the dashboard. The storybook shows all chapters and paragraphs in a book-like reading view. Each paragraph shows the date and quest that inspired it.', y);
  y += 2;
  y = addBody(doc, "The story serves as a portfolio of the child's social-emotional development, told through narrative rather than grades. Parents can read it with their child and see competency growth reflected in the adventure.", y);
  addPageNum(doc, 9);
  doc.addPage();

  // ── 11. TIPS FOR PARENTS ──────────────────────────────────────
  y = 25;
  y = addHeader(doc, "11. Tips for Parents and Teachers", y);
  y += 2;
  const parentTips = [
    ...
    "Read the My Story storybook together. It shows your child's growth through narrative, making abstract competencies concrete and personal.",
  ];
  ...
  // ── 11. TIPS FOR KIDS ─────────────────────────────────────────
  y = checkPage(doc, y, 50);
  y = addHeader(doc, "12. Tips for Young Adventurers", y);
  ...
  const kidTips = [
    ...
    "Check your storybook after each quest to see what adventure you and your guide went on!",
  ];
  ...
  // ── 12. FAQ ───────────────────────────────────────────────────
  y = 25;
  y = addHeader(doc, "13. Frequently Asked Questions", y);
  y += 2;
  const faqs = [
    ...
    ["What is My Story?", "A personalised adventure storybook that writes itself as children complete quests. Each quest generates a paragraph starring the child and their companion. Every 5 paragraphs form a chapter. The story reflects real competency growth through narrative."],
  ...
  y = 25;
  y = addHeader(doc, "14. Curriculum Alignment Reference", y);
  ...
  doc.text("GrowQuest BC  |  curriculum.gov.bc.ca/competencies  |  Version 5.6", 105, y, {align:"center"});
  addPageNum(doc, 11);
