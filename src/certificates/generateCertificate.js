...
export function generateCertificate({
  ...
  // ── Evidence Summary ─────────────────────────────────────────
  // v6.7: contexts are shown only when the app has actually captured them
  // (not yet wired up to the quest flow). The right column shows the dates
  // of the most recent quests instead — real data we can stand behind.
  y += boxH + 7;
  const leftCol = W * 0.2;
  const rightCol = W * 0.55;

  const quests = (evidenceSummary.questTitles || []).slice(0, 4);
  const questDates = (evidenceSummary.questDates || []).slice(0, 4);
  const contexts = (evidenceSummary.contexts || []).slice(0, 4);
  const showContexts = contexts.length > 0;
  const showDates = !showContexts && questDates.length > 0;
  const rightLabel = showContexts ? "DEMONSTRATED IN" : showDates ? "RECENT DATES" : "";

  doc.setFont("helvetica", "bold");
  doc.setFontSize(7);
  doc.setTextColor(...colors.primary);
  doc.text("GROWTH EVIDENCE", leftCol, y);
  if (rightLabel) doc.text(rightLabel, rightCol, y);
  ...
  if (showContexts) {
    ...
    questDates.forEach((d, i) => {
      doc.text(`\u2022  ${d}`, rightCol + 1, y + i * 4);
    });
  }

  const rightRows = showContexts ? contexts.length : showDates ? questDates.length : 0;
  const maxRows = Math.max(quests.length, rightRows);
  y += maxRows * 4 + 3;
  ...
  // ── Reflection Quote (only when an actual reflection was captured) ─
